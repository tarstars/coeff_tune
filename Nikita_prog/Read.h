...
class vec3
{
    ...
    double abs2(){return components[1]*components[1]+components[2]*components[2]+components[3]*components[3];}
};
...
//	std::map < Direction, vec3 >::iterator pdata;
    std::vector<double> v;//current row
...
//        v.push_back();
    ...
    void clear()
    {
        ...
        if (m) delete [] m;
        m=NULL;
        data.clear();phi.clear();theta.clear();v.clear();
    }
